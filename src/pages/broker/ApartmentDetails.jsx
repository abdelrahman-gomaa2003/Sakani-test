...
function BrokerApartmentDetails() {
  const { id } = useParams();
  ...
  if (!apt) {
    return (
      <div className="text-center py-5">
        ...
        <button className="btn btn-primary" onClick={() => navigate("/broker/apartments")}>العودة للعقارات</button>
      </div>
    );
  }
          ...
          <div className="d-flex flex-column gap-4">
            ...
            <div className="d-grid gap-2 rounded-4 overflow-hidden shadow-sm" style={{ gridTemplateColumns: apt.images?.length > 1 ? "2fr 1fr" : "1fr", gridTemplateRows: "300px 200px" }}>
              ...
              {apt.video_url && apt.images?.length <= 2 && (
                ...
                >
                  <div className="broker-video-container">
            ...
            {/* Full-width video below gallery when many images */}
            {apt.video_url && apt.images?.length > 2 && (
              <div
                className="broker-video-container cursor-pointer"
                onClick={() => setLightboxIndex(mediaItems.length - 1)}
        ...
        <div className="col-lg-4">
          <div className="d-flex flex-column gap-4" style={{ position: "sticky", top: 96 }}>
            ...
            <div className="bg-white p-4 rounded-4 shadow-sm d-flex flex-column gap-2" style={{ border: "1px solid rgba(0,0,0,0.04)" }}>
              <button className="btn d-flex align-items-center justify-content-center gap-2 fw-bold py-3 owner-btn-lift" style={{ background: "var(--primary)", color: "white", borderRadius: "var(--radius-lg)" }} onClick={() => navigate(`/broker/edit-apartment/${apt.id}`)}>
...
export default BrokerApartmentDetails;
