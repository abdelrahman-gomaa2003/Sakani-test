...
function EditApartment() {
  ...
  useEffect(() => {
    const loadApartment = async () => {
      ...
      setForm({
        ...
        neighborhood: data.neighborhood || "dala",
  ...
  const handleSubmit = async () => {
    ...
    setTimeout(() => { setSaving(false); setProgress(0); navigate("/broker/apartments"); }, 800);
  };
    ...
    <div className="d-flex flex-column" style={{ minHeight: "calc(100vh - 72px)" }}>
      <div className="flex-grow-1">
        <div className="p-3 p-md-4" style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div className="mb-4">
            <h1 className="fw-bold mb-1" style={{ fontSize: "1.5rem", color: "var(--on-surface, #1A1D23)" }}>تعديل العقار</h1>
            <p className="mb-0" style={{ color: "var(--on-surface-variant, #5A6370)" }}>تعديل بيانات العقار #{id}</p>
          </div>
          ...
          <div className="row g-4">
              ...
              <div className="owner-section-card p-4 p-md-5">
                ...
                <div className="d-flex flex-column gap-3">
                  <div>
                    ...
                    <input type="text" className="form-control owner-input" style={inputStyle} value={form.title} onChange={(e) => update("title", e.target.value)} maxLength={150} />
                  </div>
                  <div>
                    <label className="form-label fw-semibold small">وصف العقار</label>
                    <textarea className="form-control owner-input" style={inputStyle} rows={4} value={form.description} onChange={(e) => update("description", e.target.value)} />
                    <small className="d-block mt-1" style={{ color: "var(--on-surface-variant)", fontSize: "0.75rem" }}>{form.description.length}/2000</small>
                  </div>
              ...
              <div className="owner-section-card p-4 p-md-5 mb-4">
                ...
                <div className="d-flex flex-column gap-3">
                  ...
                  <div>
                    ...
                    <input type="text" className="form-control form-control-sm owner-input" style={inputStyle} value={form.address} onChange={(e) => update("address", e.target.value)} />
                  </div>
                  <div className="mt-2">
                ...
                <div className="d-flex flex-column gap-3">
                  <div className="row g-3">
                    ...
                    <div className="col-md-4">
                      ...
                      <input type="number" className="form-control owner-input" style={inputStyle} value={form.area} onChange={(e) => update("area", e.target.value)} />
                    </div>
                  </div>
                  <div>
                    <label className="form-label fw-semibold small text-muted">السعر الشهري (ج.م)</label>
                    <input type="number" className="form-control owner-input" style={inputStyle} value={form.price} onChange={(e) => update("price", e.target.value)} required />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="col-12">
              <div className="owner-section-card p-4 p-md-5">
                ...
                <div
                  className={`owner-upload-zone p-5 text-center mb-3 ${dragOver ? "drag-over" : ""}`}
                ...
                >
                  ...
                  <h5 className="fw-bold mb-1" style={{ fontSize: "1rem", color: "var(--on-surface, #1A1D23)" }}>اسحب الصور هنا أو اضغط للتحميل</h5>
              ...
              <div className="owner-section-card p-4 p-md-5">
                ...
                {(existingVideo && !newVideo) ? (
                  <div className="position-relative rounded-4 overflow-hidden" style={{ background: "#000" }}>
                    <video src={existingVideo} controls playsInline className="w-100" style={{ maxHeight: 300, borderRadius: "var(--radius-lg, 18px)" }} />
                    <button
                  ...
                  <div className="position-relative rounded-4 overflow-hidden" style={{ background: "#000" }}>
                    <video src={newVideoPreview} controls playsInline className="w-100" style={{ maxHeight: 300, borderRadius: "var(--radius-lg, 18px)" }} />
                    <button
                      className="position-absolute top-3 start-3 btn btn-sm d-flex align-items-center justify-content-center rounded-circle"
                      style={{ width: 32, height: 32, background: "rgba(0,0,0,0.6)", color: "white" }}
                      onClick={handleRemoveVideo}
          ...
          <div className="mt-4 p-3 p-md-4 rounded-4" style={{ background: "white", boxShadow: "0 4px 20px rgba(0,0,0,0.06)" }}>
            {saving && (
              <div className="mb-3">
                <div className="d-flex justify-content-between mb-1">
                  <small className="fw-semibold" style={{ color: "var(--on-surface-variant, #5A6370)" }}>جاري رفع البيانات...</small>
                  <small className="fw-bold" style={{ color: "var(--primary)" }}>{Math.min(Math.round(progress), 99)}%</small>
                </div>
                <div className="rounded-3 overflow-hidden" style={{ height: 6, background: "var(--border, #DDD8D0)" }}>
            ...
            <div className="d-flex flex-column flex-md-row justify-content-between align-items-center gap-3">
              <button className="btn d-flex align-items-center gap-2 px-4" style={{ border: "1px solid var(--border, #DDD8D0)", color: "var(--on-surface-variant, #5A6370)", borderRadius: "var(--radius-md, 12px)" }} onClick={() => navigate("/broker/apartments")} type="button">
                <span className="material-symbols-outlined" style={{ fontSize: 20 }}>arrow_forward</span>
                إلغاء
              </button>
              <button className="btn d-flex align-items-center gap-2 px-4 text-white fw-bold owner-btn-lift" style={{ background: "var(--primary, #6B9080)", borderRadius: "var(--radius-md, 12px)" }} onClick={handleSubmit} disabled={saving} type="button">
