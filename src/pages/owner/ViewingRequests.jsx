...
function ViewingRequests() {
  ...
  const fetchRequests = async () => {
    if (!profile) return;
    const { data } = await viewingRequestService.getByOwner(profile.id);
    setRequests(data || []);
    setLoading(false);
  };
  ...
  const handleApprove = async () => {
    ...
    setProcessing(true);
    const { error } = await viewingRequestService.updateStatus(confirmAction.id, "approved");
    if (error) {
    ...
    await notificationService.create({
      ...
      title: "تمت الموافقة على طلب المعاينة",
      message: `تمت الموافقة على طلب معاينة شقة "${confirmAction.apartments?.title}"`,
      type: "success",
      link: "/student/viewing-requests",
    });
  ...
  const handleReject = async () => {
    ...
    setProcessing(true);
    const { error } = await viewingRequestService.updateStatus(rejectId, "rejected", rejectReason.trim());
    if (error) {
    ...
    if (req) {
      await notificationService.create({
        ...
        message: `تم رفض طلب معاينة شقة "${req.apartments?.title}". السبب: ${rejectReason.trim()}`,
        type: "error",
        link: "/student/viewing-requests",
      });
    }
  ...
  const formatTime = (timeStr) => {
    ...
    return `${hour > 12 ? hour - 12 : hour}:${m} ${hour >= 12 ? "مساءً" : "صباحاً"}`;
  };
    ...
    <div className="py-3 d-flex flex-column gap-4 owner-stagger">
      <section className="owner-section" style={{ animationDelay: "0.04s" }}>
        <div className="d-flex align-items-center gap-3 mb-1">
          <span className="material-symbols-outlined" style={{ fontSize: 32, color: "var(--primary)" }}>visibility</span>
          <h2 className="fw-bold mb-0" style={{ fontSize: "1.75rem", color: "var(--on-surface, #1A1D23)" }}>طلبات المعاينة</h2>
        </div>
        <p className="mb-0" style={{ color: "var(--on-surface-variant, #5A6370)" }}>
          إدارة طلبات المعاينة الواردة من الطلاب
        </p>
      </section>
      ...
      {loading ? (
        ...
        <>
          ...
          {/* Requests List */}
          {filtered.length === 0 ? (
            <EmptyState icon="visibility_off" title="لا توجد طلبات" description="لم يتم استلام أي طلبات معاينة بعد" />
          ) : (
                      ...
                      <div className="flex-grow-1">
                        ...
                        <div className="d-flex flex-wrap gap-3 mt-2" style={{ fontSize: "0.85rem", color: "var(--on-surface-variant, #5A6370)" }}>
                          <div className="d-flex align-items-center gap-1">
                            ...
                            <span>{formatDate(req.viewing_date)}</span>
                          </div>
                          <div className="d-flex align-items-center gap-1">
                            <span className="material-symbols-outlined" style={{ fontSize: 16 }}>schedule</span>
                            <span>{formatTime(req.viewing_time)}</span>
                          </div>
                          <div className="d-flex align-items-center gap-1">
                            <span className="material-symbols-outlined" style={{ fontSize: 16 }}>info</span>
                            <span>أُرسل {formatDate(req.created_at)}</span>
                          </div>
                        </div>
                        {req.notes && (
      ...
      <ConfirmModal
        ...
        title="الموافقة على طلب المعاينة"
        message={`هل أنت متأكد من الموافقة على طلب معاينة شقة "${confirmAction?.apartments?.title}" من ${confirmAction?.profiles?.full_name}؟`}
        confirmLabel="موافقة"
        loading={processing}
      />
        ...
        <>
          ...
          <div className="position-fixed top-50 start-50 translate-middle p-4 rounded-4" style={{ background: "#fff", zIndex: 1051, width: "90%", maxWidth: 440, boxShadow: "0 20px 60px rgba(0,0,0,0.15)" }}>
            <div className="text-center mb-3">
              ...
              <h5 className="fw-bold">رفض طلب المعاينة</h5>
